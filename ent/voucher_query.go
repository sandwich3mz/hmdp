...
type VoucherQuery struct {
	...
	withGetMore *SeckillVoucherQuery
...
// QueryGetMore chains the current query on the "getMore" edge.
func (vq *VoucherQuery) QueryGetMore() *SeckillVoucherQuery {
	query := (&SeckillVoucherClient{config: vq.config}).Query()
...
func (vq *VoucherQuery) FirstID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (vq *VoucherQuery) FirstIDX(ctx context.Context) uint64 {
...
func (vq *VoucherQuery) OnlyID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (vq *VoucherQuery) OnlyIDX(ctx context.Context) uint64 {
...
func (vq *VoucherQuery) IDs(ctx context.Context) (ids []uint64, err error) {
...
func (vq *VoucherQuery) IDsX(ctx context.Context) []uint64 {
	ids, err := vq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (vq *VoucherQuery) Clone() *VoucherQuery {
	...
	return &VoucherQuery{
		...
		withGetMore: vq.withGetMore.Clone(),
...
// WithGetMore tells the query-builder to eager-load the nodes that are connected to
// the "getMore" edge. The optional arguments are used to configure the query builder of the edge.
func (vq *VoucherQuery) WithGetMore(opts ...func(*SeckillVoucherQuery)) *VoucherQuery {
	query := (&SeckillVoucherClient{config: vq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	vq.withGetMore = query
	return vq
}
...
func (vq *VoucherQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Voucher, error) {
	...
	if query := vq.withGetMore; query != nil {
		if err := vq.loadGetMore(ctx, query, nodes,
			func(n *Voucher) { n.Edges.GetMore = []*SeckillVoucher{} },
			func(n *Voucher, e *SeckillVoucher) { n.Edges.GetMore = append(n.Edges.GetMore, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (vq *VoucherQuery) loadGetMore(ctx context.Context, query *SeckillVoucherQuery, nodes []*Voucher, init func(*Voucher), assign func(*Voucher, *SeckillVoucher)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*Voucher)
	for i := range nodes {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(seckillvoucher.FieldVoucherID)
	}
	query.Where(predicate.SeckillVoucher(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(voucher.GetMoreColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "voucher_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (vq *VoucherQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(voucher.Table, voucher.Columns, sqlgraph.NewFieldSpec(voucher.FieldID, field.TypeUint64))
	_spec.From = vq.sql
